...
def comic_list_api_view(request):
    _queryset = Comic.objects.all().order_by('id')
...
#actulizar el precio de un marvel_id
@api_view(http_method_names=['PUT'])
def comic_update_price_api_view(request):          
    ...
    _price = request.data.get('price')
    if not _price:
        raise ValidationError(
            {"price": "Este campo no puede ser nulo."}
        )
    _update = Comic.objects.filter(marvel_id = _marvel_id).update(price = _price)
    if _update:
        _instance = Comic.objects.get(marvel_id = _marvel_id)
        return Response(
            data = model_to_dict(_instance), status=status.HTTP_200_OK
        )